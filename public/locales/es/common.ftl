loading = Cargando…
home-title = Panel
home-description = Bienvenido a la plataforma de orquestación Corbusier.

about-title = Acerca de
about-description = Esta es una maqueta de TypeScript/React con diseño centrado en la accesibilidad.

controls-language-label = Idioma
controls-theme-toggle-day-label = Cambiar a día
controls-theme-toggle-night-label = Cambiar a noche
controls-theme-group-label = Tema
controls-theme-day-option-label = Día
controls-theme-night-option-label = Noche
controls-display-toggle-full-label = Cambiar a vista completa
controls-display-toggle-hosted-label = Cambiar a marco alojado
controls-drawer-heading = Vista y tema
controls-drawer-close-label = Cerrar controles de vista
controls-drawer-close-button = Cerrar
controls-reset-to-device-default-label = Restablecer al valor predeterminado del dispositivo
controls-trigger-label = Controles

app-skip-to-content = Saltar al contenido de la página
nav-sidebar-label = Navegación principal
nav-zone-mainframe = MARCO PRINCIPAL
nav-zone-projects = PROYECTOS
nav-zone-system = SISTEMA
nav-dashboard = Panel
nav-my-tasks = Mis tareas
nav-ai-suggestions = Sugerencias de IA
nav-new-directive = Nueva directiva
nav-personnel = Personal
nav-reports = Informes
nav-agent-backends = Backends de agentes
nav-tool-registry = Registro de herramientas
nav-hooks-policies = Ganchos y políticas
nav-monitoring = Supervisión
nav-tenant-management = Gestión de inquilinos
nav-settings = Ajustes
nav-feedback = Comentarios

header-brand = Corbusier
header-search-label = Buscar directivas
header-search-placeholder = Buscar directivas
header-notifications-label = Notificaciones
header-user-menu-label = Menú de usuario

dashboard-demo-region-label = Demostración de chaflán
dashboard-demo-heading = Demostración de chaflán tipo tarjeta perforada
dashboard-demo-card-title = Tarjeta de tarea
dashboard-demo-card-subtitle-ltr = chamfer-md estándar (bisel superior derecho)
dashboard-demo-card-subtitle-rtl = chamfer-md estándar (bisel superior izquierdo)
dashboard-demo-blocked-title = Tarjeta bloqueada
dashboard-demo-blocked-subtitle-ltr = chamfer-md invertido (bisel superior izquierdo)
dashboard-demo-blocked-subtitle-rtl = chamfer-md invertido (bisel superior derecho)

page-dashboard = Panel
page-dashboard-sub = Resumen del espacio de trabajo.
page-my-tasks = Mis tareas
page-my-tasks-sub = Tareas asignadas al usuario en todos los proyectos.
page-task-detail = Detalle de tarea
page-task-detail-sub = Ver y editar los detalles de la tarea.
page-ai-suggestions = Sugerencias de IA
page-ai-suggestions-sub = Recomendaciones inteligentes de los backends de agentes.
page-directives = Directivas
page-directives-sub = Instrucciones permanentes y políticas para este proyecto.
page-conversations = Conversaciones
page-conversations-sub = Hilos de conversación de agentes para este proyecto.
page-conversation-detail = Conversación
page-conversation-detail-sub = Ver el hilo completo de conversación del agente.
page-projects = Proyectos
page-projects-sub = Todas las directivas y espacios de trabajo de proyectos.
page-project-landing = Resumen del proyecto
page-project-landing-sub = Resumen y acciones rápidas para este proyecto.
page-backlog = Pendientes
page-backlog-sub = Tareas sin priorizar en espera de triaje.
page-calendar = Calendario
page-calendar-sub = Vista temporal de hitos y fechas límite del proyecto.
page-kanban = Tablero Kanban
page-kanban-sub = Arrastrar y soltar tarjetas de tareas entre columnas del flujo de trabajo.
page-list-view = Vista de lista
page-list-view-sub = Listado tabular de todas las tareas de este proyecto.
page-task-detail-project = Detalle de tarea
page-task-detail-project-sub = Ver y editar esta tarea dentro del contexto del proyecto.
page-task-deps = Dependencias de tareas
page-task-deps-sub = Grafo de dependencias de esta tarea.
page-timeline = Cronograma
page-timeline-sub = Vista tipo Gantt de duraciones y dependencias de tareas.
page-settings = Ajustes
page-settings-sub = Preferencias de la aplicación y configuración de la cuenta.
page-appearance = Apariencia
page-appearance-sub = Tema, densidad de visualización y preferencias visuales.
page-auth-settings = Autenticación
page-auth-settings-sub = Métodos de inicio de sesión, tokens y políticas de sesión.
page-integrations = Integraciones
page-integrations-sub = Conexiones de servicios de terceros y claves de API.
page-workspace-settings = Espacio de trabajo
page-workspace-settings-sub = Nombre del espacio de trabajo, valores predeterminados y preferencias del equipo.
page-personnel = Personal
page-personnel-sub = Gestionar miembros del equipo y roles de acceso.
page-reports = Informes
page-reports-sub = Paneles de analítica y rendimiento.
page-agent-backends = Backends de agentes
page-agent-backends-sub = Configuraciones y puntos finales de agentes de IA registrados.
page-agent-detail = Detalle del agente
page-agent-detail-sub = Configuración y estado de este backend de agente.
page-tool-registry = Registro de herramientas
page-tool-registry-sub = Herramientas disponibles y sus políticas de acceso.
page-tool-detail = Detalle de herramienta
page-tool-detail-sub = Configuración y detalles de uso de esta herramienta.
page-hooks-policies = Ganchos y políticas
page-hooks-policies-sub = Ganchos de automatización y políticas de gobierno.
page-hook-detail = Detalle del gancho
page-hook-detail-sub = Configuración y reglas de activación de este gancho.
page-monitoring = Supervisión
page-monitoring-sub = Salud del sistema, registros y métricas de rendimiento.
page-tenant-management = Gestión de inquilinos
page-tenant-management-sub = Gestionar configuraciones de inquilinos y límites de aislamiento.
page-user-detail = Perfil de usuario
page-user-detail-sub = Ver y editar detalles y permisos del usuario.
personnel-not-found = Personal no encontrado.
personnel-detail-id = ID de personal
personnel-detail-tasks = Tareas asignadas
personnel-detail-last-active = Última actividad
personnel-activity-region = Historial de actividad
personnel-activity-heading = Historial de actividad del personal
personnel-activity-empty = No hay actividad registrada.

## Task actions (state machine)

task-action-draft = Borrador
task-action-start = Iniciar
task-action-submit-review = Enviar a revisión
task-action-pause = Pausar
task-action-mark-done = Marcar como completada
task-action-abandon = Abandonar
task-action-none = No hay transiciones disponibles desde este estado.
task-action-transition = Pasar a { $target }

## Task hierarchy

task-hierarchy-goal = Objetivo
task-hierarchy-idea = Idea
task-hierarchy-step = Paso
task-hierarchy-task = Tarea
task-hierarchy-label = Jerarquía de tareas

## Task sections

task-section-current = Tarea actual
task-section-dependencies = Dependencias
task-section-progress = Progreso
task-section-subtasks = Subtareas
task-section-source-control = Control de versiones
task-section-activity = Actividad
task-section-details = Detalles
task-section-description = Descripción
task-section-related = Tareas relacionadas

## Task components

task-subtask-heading = Subtareas
task-deps-none = Sin dependencias.
task-deps-blocked-by = Bloqueada por
task-deps-blocks = Bloquea
task-deps-subtask-progress = Progreso de subtareas
task-deps-estimate-label = Estimación:
task-branch-none = Sin rama ni PR asociados.
task-related-none = Sin tareas relacionadas.
task-not-found = Tarea no encontrada

## Task metadata

task-meta-assignee = Responsable
task-meta-due = Vencimiento
task-meta-priority = Prioridad
task-meta-estimate = Estimación
task-meta-labels = Etiquetas

## Task filters

task-filter-region = Filtros de tareas
task-filter-state = Estado
task-filter-priority = Prioridad
task-filter-project = Proyecto
task-filter-all = Todas
task-filter-reset = Restablecer filtros
task-list-region = Lista de tareas
task-list-summary = Mostrando { $shown } de { $total }
task-list-empty = Ninguna tarea coincide con los filtros actuales.

## Task card

task-card-progress = { $progress }% de subtareas completadas
task-card-subtasks =
    { $total ->
        [one] { $done }/{ $total } subtarea
       *[other] { $done }/{ $total } subtareas
    }

## Dashboard

dashboard-health-status = Estado del sistema: { $label }
dashboard-health-region = Salud del sistema
dashboard-health-last-checked = Última comprobación:
dashboard-kpi-region = Métricas clave
dashboard-activity-region = Actividad reciente
dashboard-activity-heading = Actividad reciente
dashboard-agent-region = Utilización de agentes
dashboard-agent-heading = Utilización de agentes
dashboard-agent-turn-count =
    { $count ->
        [one] turno
       *[other] turnos
    }

## Project cards

project-list-region = Lista de proyectos
project-card-lead = Responsable:
project-card-task-summary =
    { $total ->
        [one] { $total } tarea
       *[other] { $total } tareas
    } · { $inProgress } en progreso ·
    { $blocked ->
        [one] { $blocked } bloqueada
       *[other] { $blocked } bloqueadas
    }
project-status-active = Activo
project-status-inactive = Inactivo
project-status-completed = Completado

## Project landing

project-view-tabs-label = Vistas del proyecto
project-view-backlog = Pendientes
project-view-kanban = Kanban
project-view-calendar = Calendario
project-view-list = Lista
project-view-timeline = Cronograma

## Kanban board

kanban-board-label = Tablero Kanban
kanban-column-todo = Por hacer
kanban-column-planned = Planificado
kanban-column-in-progress = En progreso
kanban-column-in-review = En revisión
kanban-column-done = Hecho
kanban-add-new = Añadir nuevo

## Backlog view

backlog-col-priority = Prioridad
backlog-col-task = Tarea
backlog-col-assignee = Responsable
backlog-col-due = Vencimiento
backlog-empty-state = No hay tareas en borrador para este proyecto.

## Calendar view

calendar-grid-label = Calendario
calendar-day-no-tasks = { $date } — no hay tareas vencidas
calendar-day-no-tasks-today = { $date } — hoy — no hay tareas vencidas
calendar-day-with-tasks =
    { $date } — { $count ->
        [one] { $count } tarea vence
       *[other] { $count } tareas vencen
    }
calendar-day-with-tasks-today =
    { $date } — hoy — { $count ->
        [one] { $count } tarea vence
       *[other] { $count } tareas vencen
    }

## List view

list-col-id = ID
list-col-task = Tarea
list-col-status = Estado
list-col-priority = Prioridad
list-col-assignee = Responsable
list-col-due = Vencimiento

## Shared components

activity-timeline-label = Línea de tiempo de actividad
progress-label = Progreso
task-subtask-status-complete = completada
task-subtask-status-pending = pendiente
task-subtask-item = { $name } ({ $status })

## Conversations

conversation-list-region = Lista de conversaciones
conversation-list-empty = No hay conversaciones para este proyecto.
conversation-message-count-label =
    { $count ->
        [one] { $count } mensaje
       *[other] { $count } mensajes
    }
conversation-status-active = Activo
conversation-status-idle = Inactivo
conversation-not-found = Conversación no encontrada
conversation-timeline-label = Cronología de la conversación

## Message roles

message-role-user = Usuario
message-role-assistant = Agente
message-role-tool = Herramienta
message-role-system = Sistema

## Agent status

agent-status-label = Estado del agente
agent-status-backend = Backend
agent-status-model = Modelo
agent-turn-idle = Inactivo
agent-turn-processing = Procesando
agent-turn-awaiting-tool = Esperando resultado de herramienta

## Tool calls

tool-status-succeeded = Exitoso
tool-status-failed = Fallido
tool-status-pending = Pendiente
tool-call-id = ID de llamada
tool-call-duration = Duración
tool-call-show = Mostrar detalles
tool-call-hide = Ocultar detalles
tool-call-input = Entrada
tool-call-output = Salida

## Handoff annotations

handoff-annotation-label = Transferencia de agente
handoff-annotation-prefix = Transferencia

## Slash command input

slash-input-label = Entrada de comando
slash-input-placeholder = Escribe / para comandos…
slash-input-suggestions-label = Comandos disponibles

## Directives

directive-list-region = Lista de directivas
directive-parameters = Parámetros
directive-required = obligatorio
directive-template = Plantilla
directive-show-examples = Mostrar ejemplos
directive-hide-examples = Ocultar ejemplos
directive-example-heading = Ejemplos de expansión

## AI Suggestions

suggestion-confidence-label = { $value }% confianza
suggestion-duration = Est.
suggestion-assignees-label = Responsables sugeridos
suggestion-dismiss = Descartar
suggestion-add-backlog = Agregar al Backlog
suggestion-metric-analysed = Elementos Analizados
suggestion-metric-suggested = Tareas Sugeridas
suggestion-metric-confidence = Confianza Prom.
suggestion-metric-updated = Última Actualización
suggestion-summary-region = Resumen de sugerencias
suggestion-filter-tabs = Filtro de proyectos
suggestion-filter-all = Todos los Proyectos
suggestion-priority-high = Prioridad Alta
suggestion-priority-medium = Prioridad Media
suggestion-priority-low = Prioridad Baja
suggestion-empty = No hay sugerencias para mostrar.
suggestion-insights-region = Perspectivas de IA
suggestion-insights-heading = Perspectivas de IA
insight-severity-critical = Crítico
insight-severity-warning = Advertencia
insight-severity-info = Info

## Command palette

palette-label = Paleta de comandos
palette-placeholder = Buscar tareas, conversaciones, comandos…
palette-input-label = Buscar comandos
palette-results-label = Resultados de búsqueda
palette-no-results = No se encontraron resultados.
palette-hint-navigate = Navegar
palette-hint-open = Abrir
palette-hint-close = Cerrar
palette-group-tasks = Tareas
palette-group-conversations = Conversaciones
palette-group-commands = Comandos
palette-group-projects = Proyectos

## Notifications

notifications-heading = Notificaciones
notifications-list-label = Notificaciones recientes
notifications-unread = No leída

## User menu

user-menu-profile = Perfil
user-menu-settings = Ajustes
user-menu-tenant = Inquilino
user-menu-sign-out = Cerrar sesión

## Settings — Profile

settings-profile-heading = Perfil
settings-display-name = Nombre para mostrar
settings-email = Correo electrónico
settings-avatar = Avatar
settings-avatar-upload = Subir foto
settings-notifications-heading = Preferencias de notificación
settings-notif-email = Notificaciones por correo
settings-notif-email-desc = Recibir asignaciones de tareas y revisiones por correo.
settings-notif-hooks = Alertas de fallo de hooks
settings-notif-hooks-desc = Recibir notificación cuando falle un hook o puerta de política.

## Settings — Authentication

auth-api-keys-heading = Claves API
auth-col-name = Nombre
auth-col-key = Clave
auth-col-created = Creada
auth-col-last-used = Último uso
auth-generate-key = Generar clave
auth-api-keys-table-label = Claves API
auth-sessions-heading = Sesiones activas
auth-col-device = Dispositivo
auth-col-ip = Dirección IP
auth-col-last-active = Última actividad
auth-sessions-table-label = Sesiones activas

## Settings — Workspace

workspace-encapsulation-heading = Proveedor de encapsulación
workspace-provider-label = Proveedor
workspace-resources-heading = Límites de recursos
workspace-cpu-label = Núcleos de CPU
workspace-memory-label = Memoria (GB)
workspace-disk-label = Disco (GB)
workspace-timeout-label = Tiempo de espera de tarea (minutos)
workspace-unit-gb = GB
workspace-unit-min = min
workspace-tool-policy-heading = Política de herramientas
workspace-file-edit-policy = Política de edición de archivos
workspace-policy-ask = Preguntar antes de cada edición
workspace-policy-allow = Permitir todas las ediciones
workspace-policy-deny = Denegar todas las ediciones

## Settings — Integrations

integrations-github-heading = GitHub
integrations-gitlab-heading = GitLab
integrations-client-id = ID de cliente
integrations-client-secret = Secreto de cliente
integrations-webhook-url = URL de webhook
integrations-frankie-heading = Adaptador de revisión Frankie
integrations-frankie-endpoint = URL del endpoint
integrations-frankie-token = Token de API
integrations-test-connection = Probar conexión

## Settings — Appearance

appearance-theme-heading = Tema
appearance-density-heading = Densidad del diseño
appearance-density-label = Densidad del diseño
appearance-density-compact = Compacto
appearance-density-comfortable = Cómodo
appearance-density-spacious = Espacioso
appearance-sse-heading = Actualizaciones en tiempo real
appearance-sse-auto-reconnect = Reconexión automática al perder conexión
appearance-sse-desc = Restablecer automáticamente conexiones SSE cuando el servidor esté disponible.

## Sign In

sign-in-brand = CORBUSIER
sign-in-tagline = Plataforma de Orquestación de Ingeniería
sign-in-heading = Bienvenido de vuelta
sign-in-subtitle = Inicia sesión en tu espacio de trabajo para continuar
sign-in-email-label = Dirección de correo electrónico
sign-in-password-label = Contraseña
sign-in-password-placeholder = Ingresa tu contraseña
sign-in-remember = Recordar este dispositivo
sign-in-forgot = ¿Olvidaste tu contraseña?
sign-in-submit = Iniciar sesión en el espacio de trabajo
sign-in-no-account = ¿No tienes una cuenta?
sign-in-request-access = Solicitar acceso
sign-in-feature-orchestration = Orquestación de tareas en tiempo real
sign-in-feature-orchestration-desc = Flujos de trabajo inteligentes con agentes y actualizaciones en vivo
sign-in-feature-security = Seguridad de nivel empresarial
sign-in-feature-security-desc = RBAC, registros de auditoría y aplicación de políticas
sign-in-feature-insights = Información impulsada por IA
sign-in-feature-insights-desc = Sugerencias proactivas y recomendaciones inteligentes

## Hardcoded string fixes

sign-in-email-placeholder = you@company.com
palette-kbd-esc = esc
integrations-frankie-endpoint-placeholder = https://frankie.internal/api/v1
integrations-frankie-token-placeholder = frk_••••••••
settings-avatar-initials = AC
workspace-provider-docker = Docker
workspace-provider-podman = Podman
workspace-provider-firecracker = Firecracker
workspace-provider-gvisor = gVisor
