...
retain Bun’s performance benefits *and* enable `axe-core` scans through other
...
- **Fast feedback for logic and UI structure:** Bun’s test runner with Happy
  ...
  behaviour, state management, and basic rendering structure. Immediate
  feedback stays in the inner loop without waiting on a heavy browser
  simulation.
...
In this snippet, the Bun test quickly sets up a React root in a fake DOM,
renders a component, and makes assertions. The usage of
`document.querySelector` and standard DOM APIs is possible thanks to Happy DOM.
Tests like this run blazingly fast (no real browser, no JSDOM overhead) and can
...
- Mounts the React component into that JSDOM document (likely using ReactDOM or
...
API** (via something like `jest-axe` or `vitest-axe` utilities) can perform the
scan. The following illustrates a typical pattern, written in a Jest/Vitest
style similar to what a custom harness would do:
...
the component is *free of axe-detectable violations* but also that it adheres
...
*Automation:* Biome loads the `tools/grit/rule-testing-*.grit` patterns, which
...
- `tests/header-bar.test.tsx` should continue asserting theme state through
  the real controls, for example by checking the active button’s `aria-pressed`
...
- **3.1 In-Browser Axe Scans:** Running `axe-core` in a **real browser**
  context to catch issues like colour contrast, focus order, and other things
  JSDOM can’t detect.
...
- **Different screen sizes (responsive breakpoints):** Layouts should be
  tested on a small mobile viewport (e.g. 375×667) vs a desktop viewport (e.g.
...
- The Node axe tests can produce an HTML report of all violations, with links
...
- **Install/Verify Bun** in the project. Ensure the `bun:test` script is
  working with Happy DOM. Add a `tests/setup-happy-dom.ts` to configure any
  globals or polyfills (for example, setting `document.doctype` if needed, or
  configuring `window.fetch` if used in tests).
...
- **Add Dependencies:** Install `axe-core` (or `jest-axe`) and
  `@testing-library/react` as devDependencies for the Node tests. Also install
  `jsdom` if not already included (though some testing libraries bring it in).
  Optionally, add `tsgo` or configure `tsc` for quick compilation of tests.
...
- **Write Tests for Key Components:** Start adding tests (both Bun and a11y)
  for all crucial components in the codebase. Prioritize components that are
  reused often or have complex accessibility considerations (dialogs, menus,
  inputs, etc.). Leverage the patterns illustrated (render, axe scan, role/name
  assertions).
...
- **Visual Baseline:** Write a simple visual test for a static page and
  ...
  screenshot options if needed (such as disabling antialiasing via CSS in test).
...
- **Implement Visual & I18n Tests:** Add tests for visual snapshots of key
  ...
  either part of the main Playwright suite or a separate suite if they should
  run less frequently (Playwright can tag tests, for example with an `@visual`
  tag, to filter runs).
...
- **Backfill Tests:** Gradually increase coverage by adding tests for
  ...
  sake, but rather coverage of all **accessibility aspects** of each element.
  Use the a11y test suite as a checklist for components – if a component has an
...
- **Update Snapshots Intentionally:** When a legitimate UI change occurs,
  ...
  updates as code changes requiring review and double-check that the new output
  is correct.
...
- **Documentation by Tests:** The tests themselves serve as living
  documentation of accessibility expectations. New team members can read tests
  to understand how components should behave (for example, “Modal should return
...
- **Compliance and Confidence:** Conformance to standards (such as WCAG 2.1
  ...
  criteria (contrast, focus order, semantics, etc.). This mitigates risk from a
