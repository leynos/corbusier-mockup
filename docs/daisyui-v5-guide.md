...
4. daisyUI is suggested to be installed as a dependency, but to use it from
   CDN, include Tailwind CSS and daisyUI CDN files:
...
3. If customization of daisyUI styles using Tailwind CSS utility classes does
   ...
   `btn bg-red-500!` sets a custom background colour on a `btn` forcefully.
   This is a last resort solution and should be used sparingly.
4. If a specific component or something similar to it does not exist in
   daisyUI, create a custom component using Tailwind CSS utility classes.
...
8. For placeholder images, use <https://picsum.photos/200/300> with the
   required size.
...
- When generating a custom theme, omit the comments from the example above.
...
- `id` is required for the `drawer-toggle` input. Change `my-drawer` to a
  unique id that matches the context
